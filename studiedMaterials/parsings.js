...
const parsing = async (keyword) => {
    ...
    postElements.each(function() {
        ...
        var firstP = " " + $(this).find(".cleared > ul > li > span").eq(1).text() + " ";
        var secondP = " " + $(this).find(".cleared > ul > li > span").eq(3).text() + " ";
        var thirdP = " " + $(this).find(".cleared > ul > li > span").eq(5).text() + " ";
    
        if ((contains(firstP, first_name) == true) || (contains(secondP, first_name) == true) || contains(thirdP, first_name) == true) {
            const title = $(this).find(".extra-tight-line-height:eq(0)").text().replace(/\s+/, "").replace(/\s+$/g, "").replace(/\n/g, "").replace(/\r/g, "");
            const link = "https://www.nature.com" + $(this).find(".cleared > h3 > a").attr("href");
            const firstPN = firstP.substr(1, firstP.length - 2);
            const secondPN = secondP.substr(1, secondP.length - 2);
            const thirdPN = thirdP.substr(1, thirdP.length - 2);
            const time = $(this).find(".cleared > p > time").attr("datetime");
            sheet.data[i].push(title, link, firstPN, secondPN, thirdPN, time, i);
            i++;
        }
